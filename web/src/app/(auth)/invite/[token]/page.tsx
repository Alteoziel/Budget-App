    ...
    <main className="flex min-h-dvh items-center bg-app-glow px-5 py-10">
      <div className="mx-auto w-full max-w-md rounded-3xl border border-ink-900/5 bg-sand-50/80 p-6 shadow-soft">
        ...
        <p className="mt-2 text-sm text-ink-600">
          Someone invited you to collaborate on their budget.
        </p>
        {query.error ? (
        ...
        {user ? (
          ...
          <div className="mt-6 space-y-3">
            <Link
              ...
              className="flex w-full items-center justify-center rounded-2xl bg-ink-900 px-4 py-3.5 text-sm font-bold text-sand-50"
            >
              Sign in to accept
            </Link>
            <Link
              href={`/login?mode=signup&next=${encodeURIComponent(`/invite/${token}`)}`}
              className="flex w-full items-center justify-center rounded-2xl bg-moss-500 px-4 py-3.5 text-sm font-bold text-sand-50"
            >
